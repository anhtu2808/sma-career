    ...
    <footer style={{ background: footerConfig.backgroundColorOverride || '#1a1a2e', padding: '48px 40px', color: footerConfig.textColorOverride || '#fff' }}>
      <div className="career-container" style={{
        ...
        <div style={{ flex: 1, minWidth: '260px', maxWidth: '400px' }}>
          ...
          {contact?.email && (
            <p style={{ fontSize: `${12 * ((theme.baseFontSize || 16) / 16)}px`, color: 'inherit', opacity: 0.6, marginBottom: '8px' }}>
              ✉ {contact.email}
            </p>
          )}
          {contact?.phone && (
        ...
        <div style={{ flex: 1, minWidth: '260px', maxWidth: '400px', textAlign: 'right' }}>
          ...
          {contact?.addresses && contact.addresses.map((addr, i) => (
            <p key={`addr-${i}`} style={{ fontSize: `${13 * ((theme.baseFontSize || 16) / 16)}px`, color: 'inherit', opacity: 0.75, lineHeight: 1.6, marginBottom: '10px' }}>
              {addr}
            </p>
          ))}
        </div>
      ...
      <div style={{
        ...
        textAlign: 'center', fontSize: `${12 * ((theme.baseFontSize || 16) / 16)}px`, color: 'inherit', opacity: 0.5,
      }}>
